...
float dxLon;
/*
 FLASH: code:240100, data:112932, headers:8432   free for files:7765000
   RAM1: variables:222592, code:236440, padding:25704   free for local variables:39552
...
void DrawKeyboard() {
  ...
  tft.drawRect(30, 150, 730, 280, RA8875_WHITE);  // Outline
  ...
  while (true) {
    ...
    if (valPin != BOGUS_PIN_READ) {                        // If a button was pushed...
      ...
      switch (pushButtonSwitchIndex) {
        ...
        case UNUSED_2:  // They are all finished
          return;
...
double BearingHeading(char *dxCallPrefix) {
  ...
  if (countryIndex != -1) {              // Did we find prefix??
    ...
    tft.setTextColor(RA8875_GREEN);
    tft.setCursor(380 - (17 * tft.getFontWidth(0)) / 2, 1);  // Center city above image
    tft.print("Country not found");                          // Nope
    return -1.0;
  }
  ...
  bearingDistance = HaversineDistance(homeLat, homeLon, dxLat, dxLon);
...
int FindCountry(char *prefix) {
  ...
  while (true) {
    if (strcmp(dxCities[i].callPrefix, prefix) == 0) {  // This the one?
      return i;                                         // Yep, go home
    };

    i++;                                      // Nope, look at next entry
...
double HaversineDistance(double hLat, double hLon, double dxLat, double dxLon) {
  double dLat = (dxLat - homeLat) * DEGREES2RADIANS;
  double dLon = (dxLon - homeLon) * DEGREES2RADIANS;

  // convert to radians
  hLat = (hLat)*DEGREES2RADIANS;
  dxLat = (dxLat)*DEGREES2RADIANS;

  // apply formulae
  double a = pow(sin(dLat / 2.0), 2.0) + pow(sin(dLon / 2.0), 2.0) * cos(homeLat) * cos(dxLat);
  double earthRadius = 6376.5;  // In km
...
void bmpDraw(const char *filename, int x, int y) {
  ...
  //  bearingRadians = atan2(xR, yR);
  //  bearingDegrees *= RADIANS2DEGREES;
  ...
  for (int i = rayStart; i < rayEnd; i++) {
    tft.drawLineAngle(x1, y1, displayBearing + i, 190, RA8875_RED, -90);
  }
...
#ifdef DEBUG
  Serial.printf("\t(%d, %d, %d, %d) %p", x, y, cx, cy, pixels);
#endif
